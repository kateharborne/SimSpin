...
The file will always contain at least two HDU elements, but a maximum of 14, though the contents of this file will change dependent on the mode in which the observation has been constructed and the input parameters specified above. 
{: .fw-300 }

A summary of the maximum number of HDU extensions is shown below. This would be the result produced when a `build_datacube` observation has been run in `method="velocity"` mode and the FITS file has been written with `split_save = F`.
...
| 9   	| RAW_FLUX 	| An image of the raw particle flux in CGS in a given band specified by `filter` in the `telescope` function.                                                                                                                                                                                   	|
