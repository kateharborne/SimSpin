...
test_that("stellar ages change depending on H0", {
  ...
  expect_false(
    as.numeric(.SFTtoAge(0.3, H0 = 100)) ==
     as.numeric(.SFTtoAge(0.3, H0 = 70))
    )

  expect_false(
    all(as.numeric(.SFTtoAge(seq(0.01,1, length.out=100), H0 = 100, cores=2)) ==
      as.numeric(.SFTtoAge(seq(0.01,1, length.out=100), H0 = 70, cores = 2)))
    )
})
