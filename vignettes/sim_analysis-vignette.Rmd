...
`sim_analysis(filename, ptype = NA, bin_type = "r", rmax = 200, rbin = 200, DM_profile = NA)`
...
``` {r} 
S0 = snapshot::snapread("~/SimSpin/inst/extdata/S0_vignette")$part
```

If we run the `\sim_analysis()` function simply with the default inputs described above, 

``` {r, error=TRUE, purl=FALSE} 
S0_defaults = sim_analysis(filename = "~/SimSpin/inst/extdata/S0_vignette")
```

we receive an error. This is because the dark matter particles in the `S0_vignette` simulation provided have been replaced with an analytic description of the dark matter potential. As the function analyses all components in the simulation by default, the code issues an error. Without this mass in the form of particles, the output Gadget file contains no record of the effect that the dark matter analytic profile has on the other particles in the simulation. We need to specify this using a list, `DM_profile`, that contains details about that dark matter profile such that the total profile can be calculated properly. 
...
As the `S0_vignette` was run with a Hernquist analytic potential, we will use the second option:

``` {r} 
S0_total_3D = sim_analysis(filename   = "~/SimSpin/inst/extdata/S0_vignette",
                           DM_profile = list("profile" = "Hernquist",
                                             "DM_mass" = 184.9, 
                                             "DM_a"    = 34.5))
```

If we want to consider the radial profiles of the disk or bulge particles in isolation, the dark matter profile can be left unspecified without an error:

``` {r} 
S0_disk_3D  = sim_analysis(filename = "~/SimSpin/inst/extdata/S0_vignette",
                           ptype    = c(3))

S0_bulge_3D = sim_analysis(filename = "~/SimSpin/inst/extdata/S0_vignette",
                           ptype    = c(4))
```
...
magicaxis::magplot(S0_total_3D$r, S0_total_3D$vc, type="l", col=pal[7], lwd=3, xlim = c(0,30), xlab="radius, kpc", ylab=expression("V"[C]*", km s"^-1))
lines(S0_bulge_3D$r, S0_bulge_3D$vc, col=pal[5], lwd=3)
lines(S0_disk_3D$r, S0_disk_3D$vc, col=pal[1], lwd=3)
legend("topright", c("total", "disk", "bulge"), col=c(pal[7], pal[1], pal[5]), lty=c(1,1,1), bty="n", lwd=c(3,3,3))

magicaxis::magplot(S0_total_3D$r, S0_total_3D$lambda, type="l", col=pal[7], lwd=3, xlim = c(0,30), ylim=c(0, 2), xlab="radius, kpc", ylab=expression(paste(lambda)[R]))
lines(S0_bulge_3D$r, S0_bulge_3D$lambda, col=pal[5], lwd=3)
lines(S0_disk_3D$r, S0_disk_3D$lambda, col=pal[1], lwd=3)

```

The function also allows you to bin data in other ways - for example, if you need to bin your data radially along the plane of the disk, you could specify `bin_type = cr` to give cylindrical radial bins in the layout below:

``` {r} 
S0_total_2D = sim_analysis(filename   = "~/SimSpin/inst/extdata/S0_vignette",
                           bin_type   = "cr", 
                           DM_profile = list("profile" = "Hernquist",
                                             "DM_mass" = 184.9, 
                                             "DM_a"    = 34.5))
...
There is also the option to specify `bin_type = z` in which you consider planes above the surface of the disk, such as:

``` {r} 
S0_total_1D = sim_analysis(filename   = "~/SimSpin/inst/extdata/S0_vignette",
                           bin_type   = "z", 
                           DM_profile = list("profile" = "Hernquist",
                                             "DM_mass" = 184.9, 
                                             "DM_a"    = 34.5))
...
* the z-coordinates above the plane of the disk,
* the mass distributions,
* the `log(`density`)` profiles,
* the angular momentum components and magnitude,
* the vertical velocity and dispersion off the plane of the disk.

With this information, we can then consider mass and density profiles in different directions, for example:

```{r, echo=FALSE, fig.width=5.5, fig.height=5, fig.align="center"}
magicaxis::magplot(S0_total_3D$r, S0_total_3D$Mass, type="l", col=pal[6], lwd=3, xlim = c(0,30), xlab="radius, kpc", ylab=expression("Mass, 10"^10 *" M"['\u0298']))
lines(S0_total_2D$cr, S0_total_2D$Mass, col=pal[3], lwd=3)
lines(S0_total_1D$z, S0_total_1D$Mass, col=pal[2], lwd=3)
legend("bottomright", c("1D", "2D", "3D"), col=c(pal[2], pal[3], pal[6]), lty=c(1,1,1), bty="n", lwd=c(3,3,3))

magicaxis::magplot(S0_total_3D$r, S0_total_3D$logp, type="l", col=pal[6], lwd=3, xlim = c(0,30), xlab="radius, kpc", ylab=expression("log"[10]*"("*paste(rho)*"), 10"^10 *" M"['\u0298']*" kpc"^{-3}))
lines(S0_total_2D$cr, S0_total_2D$logp, col=pal[3], lwd=3)
lines(S0_total_1D$z, S0_total_1D$logp, col=pal[2], lwd=3)
legend("bottomright", c("1D", "2D", "3D"), col=c(pal[2], pal[3], pal[6]), lty=c(1,1,1), bty="n", lwd=c(3,3,3))
