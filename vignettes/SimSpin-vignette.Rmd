...
```{r} 
SAMI_blur = blur_cube(ifu_datacube   = SAMI_cube_phys,        # output from ifu_cube() function
...
The output of this function is in the same format as `ifu_cube()`, but with the data cube convolved such that the images produced are blurred by the degree specified by `FWHM`. If blurring has been applied, the axis ratio produced by this function then should be supplied to `find_reff()` (see the next step) in order to calculate the correctly observed ellipticity. 

``` {r, echo=FALSE, fig.width=3.25, fig.height=3.85} 
magicaxis::magimage(apply(SAMI_cube_phys$cube, c(1,2), sum), xaxt="n", yaxt="n", ann=FALSE, col=rev(colorRampPalette(RColorBrewer::brewer.pal(9, "RdYlBu"))(100)))
legend("topleft", "W/O Beam Smear", text.col = "white", bty="n", cex=0.9)
fields::image.plot(legend.only = TRUE, zlim = range(c(apply(SAMI_cube_phys$cube, c(1,2), sum))), col = rev(colorRampPalette(RColorBrewer::brewer.pal(9, "RdYlBu"))(100)), horizontal = TRUE)

magicaxis::magimage(apply(SAMI_blur$cube, c(1,2), sum), xaxt="n", yaxt="n", ann=FALSE, col=rev(colorRampPalette(RColorBrewer::brewer.pal(9, "RdYlBu"))(100)))
legend("topleft", "W/ Beam Smear", text.col = "white", bty="n", cex=0.9)
fields::image.plot(legend.only = TRUE, zlim = range(c(apply(SAMI_blur$cube, c(1,2), sum))), col = rev(colorRampPalette(RColorBrewer::brewer.pal(9, "RdYlBu"))(100)), horizontal = TRUE)
```
...
As stated above, the seeing conditions will effect the axis ratio that is recovered. Hence, if the function `ifu_blur()` has been used, the output axis ratio of this should be supplied to `find_reff()`.

```{r} 
SAMI_reff_blur = find_reff(filename   = "~/LambdaR/inst/extdata/S0_vignette", 
                           ptype      = NA,                        # considering all particles
                           r200       = 200,                       # virial radius, kpc
                           inc_deg    = 75,                        # inclination of galaxy, degrees
                           axis_ratio = SAMI_blur$axis_ratio)      # axis ratio of the galaxy, kpc
```
...
SAMI_inc_lambda = inc_correct(inc_deg      = 75,                           # true inclination, degees
                              ...
                              obs_lambdar  = SAMI_lambda$obs_lambdar) # measured lambda_R
```
...
abline(h=SAMI_lambda$obs_lambdar, col="black", lwd=3)
