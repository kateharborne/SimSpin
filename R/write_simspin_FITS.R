...
write_simspin_FITS = function(output_file, simspin_datacube, object_name,
  ...
  if (observation$method == "velocity"){
    ...
    extnames    = if (voronoi){
                      c("OBS_FLUX", "OBS_VEL", "OBS_DISP", "OBS_H3", "OBS_H4", "RESIDUAL", "OBS_MASS", "OBS_AGEM", "OBS_AGEL", "OBS_LUM",
                        "RAW_FLUX", "RAW_MASS", "RAW_VEL", "RAW_DISP", "RAW_AGEM", "RAW_AGEL", "RAW_Z", "RAW_LUM", "NPART", "VORONOI")
                    } else {
                      c("OBS_FLUX", "OBS_VEL", "OBS_DISP", "OBS_H3", "OBS_H4", "RESIDUAL", "OBS_MASS", "OBS_AGEM", "OBS_AGEL", "OBS_LUM",
                        "RAW_FLUX", "RAW_MASS", "RAW_VEL", "RAW_DISP", "RAW_AGEM", "RAW_AGEL", "RAW_Z", "RAW_LUM", "NPART")
                    }

    bunits      = if (voronoi){
                      c("erg/s/cm**2", "km/s", "km/s", "unitless", "unitless", "percentage", "Msol", "mass-weighted Gyr", "light-weighted Gyr", "Lsun",
                        "erg/s/cm**2", "Msol", "km/s", "km/s", "mass-weighted Gyr", "light-weighted Gyr", "Z_solar", "Lsun", "Particle number", "Bin ID")
                    } else {
                      c("erg/s/cm**2", "km/s", "km/s", "unitless", "unitless", "percentage", "Msol", "mass-weighted Gyr", "light-weighted Gyr", "Lsun",
                        "erg/s/cm**2", "Msol", "km/s", "km/s", "mass-weighted Gyr", "light-weighted Gyr", "Lsun", "Particle number")
                    }

    image_names = if (voronoi){
                      c("flux_image", "velocity_image", "dispersion_image", "h3_image", "h4_image",
                        "residuals", "mass_image", "ageM_image", "ageL_image", "luminosity_image",
                        "flux_image", "mass_image", "velocity_image", "dispersion_image", "ageM_image", "ageL_image",
                        "metallicity_image", "luminosity_image", "particle_image", "voronoi_bins")
                    } else {
                      c("flux_image", "velocity_image", "dispersion_image", "h3_image", "h4_image",
                        "residuals", "mass_image", "ageM_image", "ageL_image", "luminosity_image",
                        "flux_image", "mass_image", "velocity_image", "dispersion_image", "ageM_image", "ageL_image",
                        "metallicity_image", "luminosity_image", "particle_image")
                    }

    rawobs = if (voronoi){
                 c("obs", "obs", "obs", "obs", "obs", "obs", "obs", "obs", "obs", "obs",
    ...
    if (split_save){ # if writing each image to a seperate file
      for (i in 1:length(extnum)){ # for each image in the build_datacube output,
        ...
        if (i < 11){ # 2. Write the image to this new file HDU 2
      ...
      for (i in 1:length(extnum)){ # for each image in the build_datacube output,
        ...
        if (i < 11){ # Write each subsequent image to the next HDU
          Rfits::Rfits_write_image(data = simspin_datacube$observed_images[[which(names(simspin_datacube$observed_images) == image_names[i])]],
                                   filename = cube_file_name, ext=extnum[i],
                                   keyvalues = image_keyvalues, keycomments = image_keycomments,
                                   create_ext = TRUE, create_file = FALSE, overwrite_file = FALSE)
