...
.read_hdf5 = function(f, cores=1){
  ...
  if(is.null(head$RunLabel) && is.null(head$SimulationName)){
    ...
    if ("SimulationName" %in% names(head)){
      ...
      if(stringr::str_detect(stringr::str_to_lower(head$RunLabel), "gizmo")){gizmo = T}else{gizmo = F}
    }
  ...
  if (gizmo){output = .gizmo_read_hdf5(data, head, cores)}
...
.gizmo_read_hdf5 = function(data, head, cores){
  ...
  if ("PartType0" %in% groups){ # If gas particles are present in the file
    ...
    expected_names_gas = c("Coordinates", "Density", "Mass", "ParticleIDs",
                           "Metallicity", "StarFormationRate", "Velocities",
                           "SmoothingLength", "InternalEnergy")
    ...
    gas[["ElementAbundance/Oxygen"]] = NA
    gas[["ElementAbundance/Hydrogen"]] = NA
    gas = .check_names(gas) # NB: Currently tricking the check. Missing element abundances in input.

    gizmo_gas_names = c("SmoothingLength", "InternalEnergy")
    if (!all(gizmo_gas_names %in% names(gas))){
      stop("Error. Missing a necessary dataset for Gizmo PartType0. \n
           Either `SmoothingLength` or `InternalEnergy`. \n
           See https://kateharborne.github.io/SimSpin/examples/generating_hdf5.html#parttype0 for more info.")
    }
    ...
    gas_part = data.table::data.table("ID" = gas$ParticleIDs,
                                      ...
                                      "SFR" = gas$StarFormationRate*(.g_to_msol/.s_to_yr), #SFR in Msol/yr
                                      "Density" = gas$Density*.gcm3_to_msolkpc3, # Density in Msol/kpc^3
                                      "Temperature" = (2*.mass_of_proton*gas$InternalEnergy)/(3*.Boltzmann_constant), #assuming ideal gas law
                                      "SmoothingLength" = gas$SmoothingLength*.cm_to_kpc, # Smoothing length in kpc
                                      "ThermalDispersion" = sqrt((gas$InternalEnergy*.cms_to_kms)*(.adiabatic_index - 1)),
                                      "Metallicity" = gas$Metallicity,
                                      "Hydrogen" = gas$`ElementAbundance/Hydrogen`,
  ...
  if ("PartType4" %in% groups){
    ...
    expected_names_stars = c("Coordinates", "StellarInitMass", "Mass",
                             "ParticleIDs", "Metallicity",
    ...
    if(all(stars$InitialMass == 0)){ # NB: Another hack because all the Initial Masses seem to be 0?
      stars$InitialMass = stars$Mass / 10
    }
...
.check_names = function(particle_list){
  ...
  if ("StellarInitMass" %in% current_names){
    current_names[which(current_names == "StellarInitMass")] <- "InitialMass"
    names(particle_list) <- current_names
  }
